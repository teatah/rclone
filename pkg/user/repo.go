...
func (ur *UserDBRepo) Login(ctx context.Context, userRequest *UserRequest) (*User, error) {
	user, err := ur.GetUserByName(ctx, userRequest.Username)
	if err != nil {
		return nil, err
	}
...
func (ur *UserDBRepo) GetUserByName(ctx context.Context, username string) (*User, error) {
	var user User
...
func (ur *UserDBRepo) GetUserById(ctx context.Context, userId string) (*User, error) {
	var user User
	err := ur.pgPool.QueryRow(
		ctx,
